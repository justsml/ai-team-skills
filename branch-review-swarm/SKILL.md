...
When scoping diffs, searching code, or running analysis, **always exclude** these directories. They contain tool output, generated artifacts, or config — not source code under review.
...
For git diffs, use path exclusions: `git diff main...HEAD -- . ':!.cache' ':!temp' ':!tmp' ':!.claude'`
...
- **Exception — sequenced rollout (feature branches only):** Code that appears unused may be scaffolding for a multi-part rollout. Check for signals: numbered branch names (e.g. `feature/proj-123-part-2`), linked tickets referencing future work, or TODO/NOTE comments explaining the dependency. If evidence of sequenced work exists, verdict is `KEEP` with a note. If no evidence exists but it looks intentional, suggest the author add a `// TODO(PROJ-XXX): used by <next PR description>` comment so future reviewers (human and AI) understand the intent. **On `main`/`master`, this exception does not apply** — code already merged without consumers is dead code. Flag it for deletion.
